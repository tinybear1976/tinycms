...
func initTinyCMS() {
	...
	if err != nil {
		logger.LogContainer[defines.LOG_APP].Panic("init db for tinycms faile." + err.Error())
...
func initLogClientIp() {
	...
	if err != nil {
		logger.LogContainer[defines.LOG_APP].Panic("init db for logclientip faile." + err.Error())
...
func once_check_log_clientip_table_exists() {
	...
	if err != nil {
		logger.LogContainer[defines.LOG_APP].Panic("missing script file: " + filename)
		panic("missing script file: " + filename)
	}

	bytes, err := ioutil.ReadAll(f)
	if err != nil {
		logger.LogContainer[defines.LOG_APP].Panic("read script file error: " + filename)
	...
	if err != nil {
		logger.LogContainer[defines.LOG_APP].Panic("exec script file connection error: " + filename)
		panic("exec script file connection error: " + filename)
	}
	debugging.Debug_ShowSql("log_clientip_create_table", sql)
	_, err = conn.Exec(sql)
	if err != nil {
		logger.LogContainer[defines.LOG_APP].Panic("exec script file error: " + filename)
